...
import com.lianyubo.proxymodule.jobs.ProxyPageJob;
import com.lianyubo.proxymodule.jobs.ProxySerializeJob;
...
public class ProxyHttpClient extends AbstractHttpClient {
    ...
    public void startCrawl(){
            ...
            public void run() {
                while (true){
                    for (String url : ProxyPool.proxyMap.keySet()){
                         ...
                         * 首次本机直接下载代理页面
                         */
                        proxyDownloadThreadExecutor.execute(new ProxyPageJob(url, false));
                        try {
                            Thread.sleep(1000);
        ...
        new Thread((Runnable) new ProxySerializeJob()).start();
    }
